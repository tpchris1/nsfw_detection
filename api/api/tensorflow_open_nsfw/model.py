...
class OpenNsfwModel:
    ...
    """
    def __fully_connected(self, name, inputs, num_outputs, reuse=tf.AUTO_REUSE):
        return tf.layers.dense(
            inputs=inputs, units=num_outputs, name=name,
            kernel_initializer=tf.constant_initializer(
                self.__get_weights(name, "weights"), dtype=tf.float32),
            bias_initializer=tf.constant_initializer(
                self.__get_weights(name, "biases"), dtype=tf.float32), reuse=reuse)

    def __conv2d(self, name, inputs, filter_depth, kernel_size, stride=1,
                 padding="same", trainable=False, reuse=tf.AUTO_REUSE):
    ...
    def __batch_norm(self, name, inputs, training=False, reuse=tf.AUTO_REUSE):
